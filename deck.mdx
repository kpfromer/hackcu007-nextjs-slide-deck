...
import { poppins as myTheme } from "@mdx-deck/themes";
-->
...
# :computer: What We Will Learn About
...
<!--
TODO: Create blog post page /page/blog/first.js

Then other code section create a link
-->
...
Firstly create the `blog` folder
...
- 💻 **Everything is a component:** Import JSX components and render them directly in your MDX
  document.
- 🔧 **Customizable:** Decide which component is rendered for each markdown element.
- 📚 **Markdown-based:** The simplicity and elegance of Markdown remains, you interleave JSX only
  when you want to.
...
If you have used latex, it's like a much simplier version of that (no math stuff - _sort of_).

It's just a simple way to style text.
...
- Any route like `/blog/1`, `/blog/abc`, etc. will be matched by `pages/blog/[id].js`. The matched
  path parameter will be sent as a query parameter (called `id`) to the page.
...
# You can also chain multiple dynamic routes

Meaning you can have files named: `pages/post/[pid]/[comment].js`

If a user when to `/post/reddit-post-id/cool-comment` the `query` object would look like:

```js
{ "pid": "reddit-post-id", "comment": "cool-comment" }
```
...
```js subtitle="Purge css used in these files."
module.exports = {
